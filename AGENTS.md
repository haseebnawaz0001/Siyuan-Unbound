...
| `docs/` | Reference documentation: `FORK.md` (how this fork diverges from upstream), `SYNC.md` (bring-your-own-storage sync), `AI.md` (the AI subsystem, its tools and data egress), `BUILD.md` (building from source), `DEPLOY.md` (Docker / Unraid / TrueNAS), `WORKSPACE.md`, `SY-FORMAT.md`, `ENCRYPTED-NOTEBOOK.md`, `API.md` |
...
3. **Comments:** Write comments in English. This fork is English-first and diverges from upstream SiYuan here — upstream writes comments in Chinese, so expect this to be a recurring conflict when rebasing
4. **Punctuation:** Use ASCII punctuation in code comments. Chinese text that remains in the source is functional, not prose (Han-equivalence search fixtures, Chinese slash-command aliases, matching Chinese strings returned by the OS or by LLM providers) — leave it exactly as it is. For user-visible strings, do not hard-code punctuation in code; put it in the i18n language files so each locale renders its own
