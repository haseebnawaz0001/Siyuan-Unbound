...
export const openMenuPanel = (options: {
    ...
    // Data constructed by the frontend
    editData?: {
        previousID: string,
        colData: IAVColumn,
    },
    cellElements?: HTMLElement[],   // for select & date & relation & asset
    // Reuses the view data already constructed by the caller, skipping the internal fetch to avoid a
    // read/write timing race with a just-committed transaction
    data?: IAV,
    ...
    // The config/properties/sorts/filters/switcher menus only need field/view metadata, not row data;
    // skip row rendering to improve responsiveness for large views
    ...
    // Receives the view data and builds the panel DOM, binding events. Both the fetch callback path
    // and the options.data reuse path go through here
    ...
    <div class="b3-menu${options.type === "filters" ? " av__filter-panel" : ""}" ${["select", "date", "asset", "relation", "rollup"].includes(options.type) ? `style="${["select", "asset", "relation"].includes(options.type) ? "max-height: calc(100vh - 32px);display: flex;flex-direction: column;" : ""}min-width: 200px;${isMobile() ? "max-width: 90vw;" : "max-width: 50vw;"}"` : ""}>${html}</div>
        ...
        if (["select", "date", "asset", "relation", "rollup"].includes(options.type)) {
            ...
            if (options.type === "select") {
                ...
                }, Constants.TIMEOUT_LOAD);  // Wait for loading
            } else if (options.type === "relation") {
                bindRelationEvent({
                    menuElement,
                    cellElements: options.cellElements,
        ...
        avPanelElement.addEventListener("drop", (event) => {
            ...
            // Drag-reorder a relation column
        ...
        // Toggle a filter group's AND/OR (the select's change event, not dispatched via click)
        avPanelElement.addEventListener("change", (event: Event) => {
            ...
            // Re-render to sync all the read-only AND/OR labels (nodes at index >= 2 show a read-only span)
        ...
        avPanelElement.addEventListener("click", async (event: MouseEvent) => {
            ...
            while (target && target !== avPanelElement || type) {
                ...
                // toggleCombination is handled by the change event; skip it directly on click to avoid a no-op run
                if (type === "toggleCombination") {
                    break;
                }
                if (type === "close") {
                    if (!options.protyle.toolbar.subElement.classList.contains("fn__none")) {
                        // Close the asset file search first
                        hideElements(["util"], options.protyle);
                    } else if (!window.siyuan.menus.menu.element.classList.contains("fn__none")) {
                        // The filter panel closes the filter condition first
                    } else {
                        closeCB?.();
                        avPanelElement.remove();
                        setTimeout(() => {
                            focusBlock(options.blockElement);
                        }, Constants.TIMEOUT_TRANSITION);  // For single-select, using Enter to change an option triggers scrolling
                    }
                    window.siyuan.menus.menu.remove();
                    event.preventDefault();
                    event.stopPropagation();
                    ...
                    // After duplicating a column, clicking back to the properties panel results in a
                    // width mismatch, so it needs to be recomputed
                    ...
                    // Keep the local state's invariant of "a single empty root group at the top level"
                    // (the backend normalizes it the same way), to avoid a later addFilterGroup
                    // mistakenly treating a new group as the root group
                    ...
                    if (target.dataset.newType !== target.dataset.oldType) {
                        ...
                        // A line number column's filters and sorts need to be cleared
                        if (target.dataset.newType === "lineNumber") {
                            ...
                            if (filterExist) {
                                const oldFilters = JSON.parse(JSON.stringify(data.view.filters));
                                // Recursively remove leaves referencing this column and prune empty
                                // groups. Under spec 5 the top level is a root group, so its children
                                // are operated on; as a fallback for old flat data (no root group), the
                                // top level is handled directly.
...
<svg><use xlink:href="#iconEye${isShow ? "" : "off"}"></use></svg>`;
                ...
                // There have been error logs but no reproduction steps found, so check this first
    ...
    // When the caller-provided data is reused, render directly and skip the fetch, to avoid a
    // read/write race with a just-committed transaction (which could return stale data)
    if (options.data) {
