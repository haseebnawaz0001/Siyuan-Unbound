...
内核记录的信息并非全部落在工作区根目录下。`~/.config/siyuan/`（各平台路径统一）是一个用户级配置目录，保存的是描述这台机器和已知工作区集合的信息，而不属于任何单个工作区的内容：

| 文件 | 说明 |
|---|---|
| `workspace.json` | 已知工作区目录列表，以及最近一次打开的是哪一个 |
| `port.json` | 每个正在运行的工作区所占用的内核端口，用于发现正在运行的实例 |
| `cookie.key` | 用于签名会话 cookie 的密钥 |
| `device.id` | 本机的设备标识，用于协调云同步锁 |

这四个文件都不属于工作区：复制、移动或还原工作区目录不会带上它们中的任何一个，也不需要它们才能在同一台机器上继续工作——内核会在下次启动时从 `~/.config/siyuan/` 重新生成或找回所需内容。

`device.id` 值得多说几句，因为很容易以为它应该放在 `conf/conf.json` 里。它必须放在工作区之外，正是为了避免复制工作区目录时把设备身份也一并带到另一台机器上：如果标识跟着工作区走，两台机器共享同一份工作区副本时会向云同步呈现相同的设备 ID，而同步锁的判定逻辑是「锁的持有者是自己的设备 ID 就视为空闲、可以获取」——于是两台机器可能同时向云端写入。`conf/conf.json` 与该文件的关系见 §6。
...
其中 `system.id` 字段只是设备标识的缓存，并非权威来源：真正的值保存在工作区之外的 `~/.config/siyuan/device.id` 中（见 §2 「工作区之外：用户配置目录」小节）。只有在 `device.id` 无法读取或写入时，`system.id` 才会作为兜底被使用。
