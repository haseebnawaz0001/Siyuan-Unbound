...
func getEmbedBlock(c *gin.Context) {
	...
	headingMode := 0 // 0: show the heading and the blocks below it, 1: heading only, 2: only the blocks below the heading
...
func searchRefBlock(c *gin.Context) {
	...
	// Block-ref search within an encrypted notebook goes through the InBox variant (only searches that box's own encrypted db, preventing references across encryption boundaries)
...
func fullTextSearchBlock(c *gin.Context) {
	...
	var docMode bool
	// Full-text search for an encrypted notebook goes through the InBox variant (queries the encrypted content db + blocks_fts)
	if notebook, ok := arg["notebook"].(string); ok && notebook != "" && model.IsEncryptedBox(notebook) {
...
func parseSearchBlockArgs(arg map[string]any) (page, pageSize int, query string, paths, boxes []string, types, subTypes map[string]bool, method, orderBy, groupBy int) {
	...
	if nil != pathsArg {
		for _, p := range pathsArg.([]any) {
			...
			// Input validation: reject invalid notebook IDs and document paths containing SQL metacharacters, to
			// prevent SQL injection.
			// Consistent with the existing silent-dedup style: an invalid entry is dropped entirely rather than
	...
	// orderBy: 0: by block type (default), 1: by creation time ascending, 2: by creation time descending, 3: by
	// update time ascending, 4: by update time descending, 5: by content order (only when grouped by document),
	// 6: by relevance ascending, 7: by relevance descending
...
func parseSearchAssetContentArgs(arg map[string]any) (page, pageSize int, query string, types map[string]bool, method, orderBy int) {
	...
	// orderBy: 0: by relevance descending, 1: by relevance ascending, 2: by update time ascending, 3: by update time descending
	orderByArg := arg["orderBy"]
	if nil != orderByArg {
		orderBy = int(orderByArg.(float64))
	}
