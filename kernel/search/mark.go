...
func MarkText(text string, keyword string, beforeLen int, caseSensitive bool) (pos int, marked string) {
	...
	var before []rune
	var count int
	for i := pos; 0 < i; { // shorten it if the text before the keyword is too long
...
func EncloseHighlighting(text string, keywords []string, openMark, closeMark string, caseSensitive, splitWords bool) (ret string) {
	...
	for i, k := range keywords {
		...
		if !util.SearchHanSensitive {
			// Traditional/simplified insensitive: expand the keyword character by character into
			// traditional/simplified equivalence character classes, e.g. "诗经" -> "[诗詩][经經]"
