...
func RenderGroupView(attrView *av.AttributeView, view, groupView *av.View, query string) (ret av.Viewable) {
	...
	case av.LayoutTypeTable:
		// A deep copy is needed here because a field may carry a calculation (FieldCalc), and each grouped view's
		// calculation result must be stored on its own separate field instance
...
func RenderView(attrView *av.AttributeView, view *av.View, query string, ignoreRows bool) (ret av.Viewable) {
	...
	// Don't write to the cache when ignoreRows is set, otherwise a second render of the same view.ID by
	// genAttrViewGroups within the same request would get an empty table
	if !ignoreRows {
...
// compileTemplateField parses the template content and returns a reusable compiled template, to avoid re-parsing it repeatedly while rendering row by row.
...
func executeTemplateField(tpl *template.Template, ial map[string]string, keyValues []*av.KeyValues) (ret string, err error) {
	...
	buf := &bytes.Buffer{}
	dataModel := map[string]any{} // Copy the IAL to avoid mutating the original data
	for k, v := range ial {
		dataModel[k] = v
		...
		if nil == parseErr {
			...
			if strings.Contains(errMsg, "minute out of range") {
				...
				// Zero out the minute and second parts
...
func generateAttrViewItems(attrView *av.AttributeView, view *av.View) (ret map[string][]*av.KeyValues) {
	...
	// For a grouped view, filter out items that aren't in the group
...
func fillAttributeViewBaseValue(baseValue *av.BaseValue, fieldID, itemID string, fieldNumberFormat av.NumberFormat, fieldTemplate string, fieldDateIsTime bool) {
	...
	case av.KeyTypeNumber: // Format the number
	...
	case av.KeyTypeTemplate: // Render the template field
	...
	case av.KeyTypeCreated: // Fill in the created-time field value; rendered later
		baseValue.Value = &av.Value{ID: baseValue.ID, KeyID: fieldID, BlockID: itemID, Type: av.KeyTypeCreated}
	case av.KeyTypeUpdated: // Fill in the updated-time field value; rendered later
...
func fillAttributeViewAutoGeneratedValues(attrView *av.AttributeView, collection av.Collection, ials map[string]map[string]string, depth *int, cachedAttrViews map[string]*av.AttributeView) {
	// Render the primary key, created time, and updated time
	...
	for _, item := range collection.GetItems() {
		for _, value := range item.GetValues() {
			...
			switch value.Type {
			case av.KeyTypeBlock: // The primary key may need a static anchor text filled in: Database-bound block primary key supports setting static anchor text https://github.com/siyuan-note/siyuan/issues/10049
				if nil != value.Block {
			...
			case av.KeyTypeCreated: // Render the created time
			...
			case av.KeyTypeUpdated: // Render the updated time
				key, _ := attrView.GetKey(value.KeyID)
				isNotTime := false
...
func fillAttributeViewTemplateValues(attrView *av.AttributeView, view *av.View, collection av.Collection, ials map[string]map[string]string) (err error) {
	...
	// Cache the compiled template keyed by template content, to avoid re-parsing it repeatedly row by row. Bad
	// content is cached as nil, preserving the original semantics of "set err row by row and continue".
	...
	for _, templateKey := range templateKeys {
		for _, item := range collection.GetItems() {
			...
			if !tried {
				...
				tplCache[value.Template.Content] = compiled // When the content is bad, compiled is nil, but it's still cached
				if nil != compileErr {
				...
				renderErr = compileErrCache[value.Template.Content] // Reuse the error from the first parse
			}
			if nil == renderErr {
...
// filterByQuery filters based on the search condition
func filterByQuery(query string, collection av.Collection) {
	query = strings.TrimSpace(query)
	if "" != query {
		query = strings.Join(strings.Fields(query), " ") // Collapse consecutive whitespace into a single space
		keywords := strings.Split(query, " ")            // Split keywords on whitespace
...
// manualSort handles the user's manual sort order.
func manualSort(view *av.View, collection av.Collection) {
	itemIDs := view.ItemIDs
	// For a grouped view, sort according to the order of the grouped items
...
func GetTemplateKeyRelevantKeys(attrView *av.AttributeView, templateKey *av.Key) (ret []*av.Key) {
	...
	if 1 > len(ret) {
		// When there are no related fields, try parsing the template directly; if it parses successfully, return the template field itself https://github.com/siyuan-note/siyuan/issues/15560#issuecomment-3182691193
		goTpl := template.New("").Delims(".action{", "}")
		tplFuncMap := filesys.BuiltInTemplateFuncs()
		SQLTemplateFuncs(&tplFuncMap)
		goTpl = goTpl.Funcs(tplFuncMap)
