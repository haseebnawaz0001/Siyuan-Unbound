...
func renderBlockDOMByNode(node *ast.Node, luteEngine *lute.Lute) string {
	...
	ast.Walk(node, func(n *ast.Node, entering bool) ast.WalkStatus {
		if entering {
			if n.IsBlock() {
				// No need to render the database badge in a content image https://github.com/siyuan-note/siyuan/issues/15057
				ial := parse.IAL2Map(n.KramdownIAL)
				delete(ial, av.NodeAttrNameAvs)

				// Reset the data-node-id value, to avoid triggering frontend-bound events https://github.com/siyuan-note/siyuan/issues/15088
				newID := ast.NewNodeID()
				resetIDs[newID] = n.ID
				...
				if ast.NodeIFrame == n.Type || ast.NodeAudio == n.Type || ast.NodeVideo == n.Type {
					// Disable automatic playback: Disable automatic video playback in database card view https://github.com/siyuan-note/siyuan/issues/15212
	...
	ast.Walk(node, func(n *ast.Node, entering bool) ast.WalkStatus {
		if entering {
			if n.IsBlock() {
				// Restore the data-node-id value reset above
				ial := parse.IAL2Map(n.KramdownIAL)
				oldID := resetIDs[n.ID]
